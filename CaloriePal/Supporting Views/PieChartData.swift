...
class PieChartData: ObservableObject {
    @Published private(set) var data: [PieChartSlideData] = []
    
    init(data: [Double], colors: [Color] = []) {
        var currentAngle: Double = -90
        var slides: [PieChartSlideData] = []
        let total = data.reduce(0.0, +)
        
        for index in 0..<data.count {
            let slide = PieChartSlideData()
            let dataItem = colors.isEmpty ?
                PieChartDataItem(name: "Data name \(index + 1)", value: data[index]) :
                PieChartDataItem(name: "Data name \(index + 1)", value: data[index], color: colors[index])
